...
export function startKsonServer(
    ...
    // Unwrap pushed settings: VSCode's push model sends `{ <ns>: {...} }`, but
    // non-VSCode clients may push an already-scoped payload. Accept either.
    function unwrapPushedSettings(settings: any): any {
        return settings?.[configNamespace] ?? settings;
    }
    ...
    connection.onDidChangeConfiguration(async (change) => {
        if (hasConfigurationCapability) {
            ...
            const configuration = ksonSettingsWithDefaults(unwrapPushedSettings(change.settings));
            textDocumentService.updateConfiguration(configuration);
        }

        const scoped = unwrapPushedSettings(change.settings);
