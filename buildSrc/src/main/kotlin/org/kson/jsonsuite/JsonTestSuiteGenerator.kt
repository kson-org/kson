...
class JsonTestSuiteGenerator(
    ...
    val generatedSchemaDraft7SuiteTestPath: Path =
        testClassPackageDir.resolve("$draft7TestClassName.kt")
    val draft2020_12ClassName = "SchemaDraft2020_12SuiteTest"
    val generatedSchemaDraft2020_12SuiteTestPath: Path =
        testClassPackageDir.resolve("$draft2020_12ClassName.kt")
    ...
    fun generate() {
        ...
        generatedSchemaDraft7SuiteTestPath.toFile().writeText(
            generateSchemaSuiteTestClasses(
                this.javaClass.name,
                classPackage,
                draft7TestClassName,
                schemaDraft7TestDataList))

        val schemaDraft2020_12TestDataList = SchemaTestDataLoader(schemaTestSourceFilesDir, "draft2020-12", projectRoot).loadTestData()
        generatedSchemaDraft2020_12SuiteTestPath.toFile().writeText(
            generateSchemaSuiteTestClasses(
                this.javaClass.name,
                classPackage,
                draft2020_12ClassName,
                schemaDraft2020_12TestDataList))
    }
}
...
${    tests.joinToString("\n") {
        ...
        for (schema in it.schemaTestGroups) {
            ...
            for (test in schema.tests) {
                ...
                val schemaTestId = "${it.testFileName}::${schema.description}::${test.description}"
                val testCode = """
                    |    /**
