...
class RustGen : LanguageSpecificBindingsGenerator {
    ...
    override fun generate(packageMetadata: SimplePackageMetadata): String {
        ...
        metadata.classes.forEach {
            if (it.value.isSealed()) {
                // The class is sealed, so we can translate it as a Rust enum
                generateEnum(it.value)
            } else if (metadata.sealedSubclasses.contains(it.key)) {
                // The class is a sealed subclass, so its code will be generated by the parent
            } else {
    ...
    private fun generateEnum(metadata: SimpleClassMetadata) {
        // Each variant gets its own struct
        metadata.sealedSubclasses.forEach { subclassName ->
            val subclassMetadata = this.metadata.classes[subclassName.fullyQualifiedName()]!!
            generateClass(subclassMetadata)
        }
        ...
        // Each entry wraps the corresponding struct
        for (subclassName in metadata.sealedSubclasses) {
            val subclassMetadata = this.metadata.classes[subclassName.fullyQualifiedName()]!!
            val subclassDocString = RustDocStringFormatter.format("  ", subclassMetadata.docString)
            builder.append(subclassDocString)
            builder.append("  ${subclassName.unqualifiedName()}(${subclassName.unqualifiedName(IncludeParentClass.Yes(""))}),\n")
        }
        ...
        builder.append("""|
            ...
            |    static CONSTRUCTOR_MAP: std::sync::LazyLock<Vec<(util::KotlinType, fn(kson_ffi::kson_KNativePtr) -> $unqualifiedName)>> = std::sync::LazyLock::new(|| {
            |       vec![
            |""".trimMargin())
        metadata.sealedSubclasses.forEach {
            val type = "util::KotlinType { inner: unsafe { KSON_SYMBOLS.kotlin.root.${it.javaClassName()}._type.unwrap()() } }"
            val constructorFn = "|p| { $unqualifiedName::${it.unqualifiedName()}(${it.unqualifiedName(IncludeParentClass.Yes(""))}::from_kotlin_object(p)) }"
            builder.append("           ($type, $constructorFn),\n")
        }
        builder.append("""
            |       ]
            |    });
        ...
        for (subclass in metadata.sealedSubclasses) {
            builder.append("""
                |      $unqualifiedName::${subclass.unqualifiedName()}(inner) => inner.to_kotlin_object(),
                |
            """.trimMargin())
        }
        builder.append("""
    ...
    private fun generateCLikeEnum(metadata: SimpleEnumMetadata) {
        val unqualifiedName = metadata.name.unqualifiedName()
        val docString = RustDocStringFormatter.format("", metadata.docString)
        builder.append("""
            |#[derive(Copy, Clone)]
    ...
    private fun generateClass(metadata: SimpleClassMetadata, enableUpcasting: Boolean = false) {
        // Wrapper struct
    ...
    fun translateReturnExpr(returnType: SimpleType) {
        ...
        } else if (this.metadata.enums.containsKey(returnType.classifier) || metadata.classes[returnType.classifier]?.isSealed() == true) {
