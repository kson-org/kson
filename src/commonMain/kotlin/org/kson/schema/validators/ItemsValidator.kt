...
class ItemsValidator(
    private val itemsValidator: LeadingItemsValidator,
    private val additionalItemsValidator: AdditionalItemsValidator?,
) : JsonArrayValidator() {
    override fun validateArray(
        ...
        val remainingItems =
            if (validatedCount < node.elements.size) {
...
data class LeadingItemsTupleValidator(
    private val schemas: List<JsonSchema>,
) : LeadingItemsValidator {
    override fun validateArray(
        list: KsonList,
        messageSink: MessageSink,
    ): Int {
...
data class LeadingItemsSchemaValidator(
...
data class AdditionalItemsBooleanValidator(
    ...
    override fun validateArray(
        ...
        if (!allowed && remainingItems.isNotEmpty()) {
            messageSink.error(
                location,
                MessageType.SCHEMA_ADDITIONAL_ITEMS_NOT_ALLOWED.create(
...
data class AdditionalItemsSchemaValidator(
