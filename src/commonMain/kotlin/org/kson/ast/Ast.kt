...
interface AstNode {
    ...
    ): String = toSourceWithNext(indent, null, compileTarget)
    ...
    data class Indent(
        ...
        fun bodyLinesIndent(): String = indentString.repeat(indentLevel)

        /**
         * Produce a copy of this indent with the given [hanging] value for its [hanging]
         */
        fun clone(hanging: Boolean): Indent = Indent(indentType, indentLevel, hanging)

        /**
         * Produce the "next" indent in from this one, with the given [hanging] value for its [hanging]
         */
        fun next(hanging: Boolean): Indent = Indent(indentType, indentLevel + 1, hanging)
    }
}
 ...
 */
sealed class AstNodeImpl(
    override val sourceTokens: List<Token>,
) : AstNode {
     ...
     * [CompileTarget]
     */
    override fun toSourceWithNext(
        indent: Indent,
        nextNode: AstNode?,
        compileTarget: CompileTarget,
    ): String =
        if (compileTarget.preserveComments && this is Documented && comments.isNotEmpty()) {
     ...
     */
    protected abstract fun toSourceInternal(
        indent: Indent,
        nextNode: AstNode?,
        compileTarget: CompileTarget,
    ): String
 ...
 */
open class AstNodeError(
    sourceTokens: List<Token>,
) : AstNodeImpl(sourceTokens),
    AstNode {
...
class KsonValueNodeError(
    sourceTokens: List<Token>,
) : AstNodeError(sourceTokens),
    KsonValueNode

abstract class KsonValueNodeImpl(
    sourceTokens: List<Token>,
) : AstNodeImpl(sourceTokens),
    KsonValueNode
        ...
        when (compileTarget) {
            is Kson, is Yaml -> {
                ...
                trailingContent.forEachIndexed { index, trailingValue ->
                    ...
                    ksonDocument +=
                        trailingValue.toSourceWithNext(
                            indent,
 ...
 */
private fun isTrailingContent(nextNode: AstNode?): Boolean = nextNode is KsonValueNode
...
class ObjectNode(
    ...
    private fun formatDelimitedObject(
        indent: Indent,
        nextNode: AstNode?,
        compileTarget: CompileTarget,
    ): String {
    ...
    private fun formatCompactObject(
        ...
        val outputObject =
            properties.withIndex().joinToString("") { (index, property) ->
                val nodeAfterThisChild = properties.getOrNull(index + 1) ?: nextNode
    ...
    private fun formatUndelimitedObject(
        indent: Indent,
        nextNode: AstNode?,
        compileTarget: CompileTarget,
    ): String {
     ...
     */
    private fun needsEndDot(nextNode: AstNode?): Boolean = nextNode is ObjectPropertyNode || isTrailingContent(nextNode)
}

interface ObjectKeyNode : StringNode

class ObjectKeyNodeError(
    sourceTokens: List<Token>,
) : AstNodeError(sourceTokens),
    ObjectKeyNode
...
class ObjectPropertyNodeImpl(
    ...
    private fun delimitedObjectProperty(
        indent: Indent,
        nextNode: AstNode?,
        compileTarget: CompileTarget,
    ): String {
        val delimitedPropertyIndent =
    ...
    private fun undelimitedObjectProperty(
        indent: Indent,
        nextNode: AstNode?,
        compileTarget: CompileTarget,
    ): String =
    ...
    private fun compactObjectProperty(
        indent: Indent,
        nextNode: AstNode?,
        compileTarget: CompileTarget,
    ): String {
...
class ListNode(
    ...
    private sealed class ListDelimiters(
        val open: Char,
        val close: Char,
    ) {
    ...
    private fun formatCompactList(
        ...
        elements.withIndex().joinToString(
            ...
            val nodeAfterThisChild = elements.getOrNull(index + 1) ?: nextNode
            val elementString =
                if (element is Documented && element.comments.isNotEmpty()) {
                    "\n"
                } else {
                    ""
    ...
    private fun formatUndelimitedList(
        indent: Indent,
        nextNode: AstNode?,
        compileTarget: CompileTarget,
    ): String =
     ...
     */
    private fun needsEndDash(nextNode: AstNode?): Boolean = nextNode is ListElementNode || isTrailingContent(nextNode)
}

interface ListElementNode : AstNode

class ListElementNodeError(
    sourceTokens: List<Token>,
) : AstNodeError(sourceTokens),
    ListElementNode

class ListElementNodeImpl(
    val value: KsonValueNode,
    override val comments: List<String>,
    sourceTokens: List<Token>,
) : AstNodeImpl(sourceTokens),
...
abstract class StringNodeImpl(
     ...
     */
    private fun renderAsEmbedBlock(
        indent: Indent,
        compileTarget: Kson,
        embedTag: String?,
    ): String {
        ...
        return EmbedBlockRenderer.renderKsonEmbedBlock(
            ...
            preamble,
            indent,
            compileTarget.formatConfig.formattingStyle,
     ...
     */
    internal fun tryRenderAsEmbedBlock(
        indent: Indent,
        compileTarget: Kson,
    ): String? {
        val matchingRule =
            compileTarget.getEmbedRule(this)
                ?: return null
...
open class UnquotedStringNode(
    ...
    override fun toSourceInternal(
        ...
        nextNode: AstNode?,
        compileTarget: CompileTarget,
    ): String = renderUnquotableKsonString(rawStringContent, indent, compileTarget)
}

private val yamlReservedKeywords =
    setOf(
        // Boolean true values
        "y",
        "Y",
...
private fun StringNodeImpl.renderUnquotableKsonString(
    ...
    return when (compileTarget) {
        ...
        is Yaml -> {
            indent.firstLineIndent() +
                if (yamlReservedKeywords.contains(unquotedKsonString)) {
...
class EmbedBlockNode(
     ...
     */
    private val rawEmbedTag: String =
        embedTagNode?.delimiterUnescapedRawContent?.let {
