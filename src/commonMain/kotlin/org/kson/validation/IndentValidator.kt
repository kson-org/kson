...
class IndentValidator {
     ...
     */
    private fun validateNodeNesting(
        node: KsonValueNode,
            ...
            is ObjectNode -> {
                node.properties.forEach { property ->
                    ...
                    if (property is ObjectPropertyNodeImpl) {
                        validateNodeNesting(
                            property.value,
                            property.key.location.start.column + 1,
                            OBJECT_PROPERTY_NESTING_ISSUE,
                            messageSink,
                        )
                    }
            ...
            is ListNode -> {
                node.elements.forEach { element ->
                    ...
                    if (element is ListElementNodeImpl) {
                        val minListNestingColumn =
                            if (element.location.startOffset < element.value.location.startOffset) {
     ...
     */
    private fun validateNodeAlignment(
        node: KsonValueNode,
    ...
    private fun validateObjectAlignment(
        objNode: ObjectNode,
        previousNodeLine: Int,
        messageSink: MessageSink,
    ) {
    ...
    private fun validateListAlignment(
        listNode: ListNode,
        previousNodeLine: Int,
        messageSink: MessageSink,
    ) {
        validateAlignment(
            ...
            if (element is ListElementNodeImpl) {
                ...
                val prevLineNum =
                    if (value is ObjectNode || value is ListNode) {
